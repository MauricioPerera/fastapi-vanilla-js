...
const GROUP_ACCUMULATORS = {
  ...
  $sum:   (docs, f) => docs.reduce((s, d) => s + (Number(_getNestedValue(d, f)) || 0), 0),
  $avg:   (docs, f) => {
    const vals = docs.map(d => Number(_getNestedValue(d, f)) || 0);
    return vals.length > 0 ? vals.reduce((a, b) => a + b, 0) / vals.length : 0;
  },
  $min:   (docs, f) => Math.min(...docs.map(d => Number(_getNestedValue(d, f)) || Infinity)),
  $max:   (docs, f) => Math.max(...docs.map(d => Number(_getNestedValue(d, f)) || -Infinity)),
  $push:  (docs, f) => docs.map(d => _getNestedValue(d, f)),
  $first: (docs, f) => _getNestedValue(docs[0], f),
  $last:  (docs, f) => _getNestedValue(docs[docs.length - 1], f),
};
