...
// Operadores de comparación: cada fn devuelve true si la condición SE CUMPLE.
// Mitigación ReDoS: compila un patrón de regex de USUARIO rechazando los catastróficos más comunes
// (cuantificador anidado, p.ej. /(a+)+$/) y los excesivamente largos. NO es defensa completa: no caza
// la alternación solapada (p.ej. /(a|a)*/) y el fix real sería un motor lineal tipo RE2 (no zero-dep).
// Reduce la superficie de DoS por filtros $regex controlados por el cliente.
const _REDOS_HEURISTIC = /\([^()]*[*+][^()]*\)\s*[*+{]/;
function _safeUserRegex(target) {
  if (typeof target !== 'string') return target; // RegExp ya construido por código de confianza
  if (target.length > 1000) throw new Error('patrón regex demasiado largo');
  if (_REDOS_HEURISTIC.test(target)) throw new Error('patrón regex potencialmente catastrófico (ReDoS) rechazado');
...
const MATCH_OPERATORS = {
  ...
  $in:       (val, target) => Array.isArray(target) && target.includes(val),
  $nin:      (val, target) => !(Array.isArray(target) && target.includes(val)),
  $exists:   (val, target) => (val !== undefined) === target,
  $regex:    (val, target) => _safeUserRegex(target).test(String(val ?? '')),
  $contains: (val, target) => Array.isArray(val) && val.includes(target),
  $size:     (val, target) => Array.isArray(val) && val.length === target,
};

// Operadores lógicos: cada fn devuelve true si el sub-filtro SE CUMPLE.
const LOGICAL_OPERATORS = {
  $and: (doc, sub) => Array.isArray(sub) && sub.every(s => matchFilter(doc, s)),
  $or:  (doc, sub) => Array.isArray(sub) && sub.some(s => matchFilter(doc, s)),
  $not: (doc, sub) => !matchFilter(doc, sub),
};

/** Evalúa una condición de campo: RegExp directa, igualdad simple, u objeto de operadores. */
function _matchValue(val, cond) {
  ...
  for (const op of Object.keys(cond)) {
    const fn = MATCH_OPERATORS[op];
    if (fn && !fn(val, cond[op])) return false;
  }
  return true;
}
...
function matchFilter(doc, filter) {
  ...
  for (const key of Object.keys(filter)) {
    if (key in LOGICAL_OPERATORS) {
      if (!LOGICAL_OPERATORS[key](doc, filter[key])) return false;
      continue;
    }
    if (!_matchValue(_getNestedValue(doc, key), filter[key])) return false;
  }
  return true;
}
...
// Claves que permitirían prototype pollution al escribir por nombre controlado por el usuario.
const _UNSAFE_KEYS = new Set(['__proto__', 'constructor', 'prototype']);

function _setNestedValue(obj, path, value) {
  const parts = path.split('.');
  for (const p of parts) if (_UNSAFE_KEYS.has(p)) return; // anti prototype-pollution
...
function _deleteNestedValue(obj, path) {
  const parts = path.split('.');
  for (const p of parts) if (_UNSAFE_KEYS.has(p)) return; // anti prototype-tampering ($unset/$rename)
...
// Operadores de update: cada fn muta `result` aplicando `fields`.
const UPDATE_OPERATORS = {
  $set:   (result, fields) => { for (const [k, v] of Object.entries(fields)) _setNestedValue(result, k, v); },
  $unset: (result, fields) => { for (const k of Object.keys(fields)) _deleteNestedValue(result, k); },
  $inc:   (result, fields) => {
    for (const [k, v] of Object.entries(fields)) _setNestedValue(result, k, (_getNestedValue(result, k) || 0) + v);
  },
  $push:  (result, fields) => {
...
function applyUpdate(doc, update) {
  const result = _clone(doc);
  for (const op of Object.keys(update)) {
    const fn = UPDATE_OPERATORS[op];
    if (fn) { fn(result, update[op]); continue; }
    // Sin operador ($...): reemplazo completo del documento (preservando _id).
    if (!op.startsWith('$')) {
      const id = result._id;
      // Asignación segura: omite claves que contaminarían el prototipo.
      for (const [k, v] of Object.entries(update)) {
        if (!_UNSAFE_KEYS.has(k)) result[k] = v;
      }
      result._id = id;
      return result;
    }
...
class FileStorageAdapter {
  ...
  writeJson(filename, data) {
    ...
    this.fs.renameSync(tmp, file); // rename atómico: nunca deja el .json truncado ante un crash
  }
  delete(filename) {
    const file = this.path.join(this.dir, filename);
...
// Lookup sobre HashIndex: devuelve ids (posible []) o null si la condición no aplica.
function _hashIndexLookup(index, cond) {
  if (cond === null || typeof cond !== 'object') return index.lookup(cond);
  if (cond.$eq !== undefined) return index.lookup(cond.$eq);
  if (cond.$in && Array.isArray(cond.$in)) {
    const ids = new Set();
    for (const v of cond.$in) {
      for (const id of index.lookup(v)) ids.add(id);
    }
...
// Lookup sobre SortedIndex: rangos ($gt/$gte/$lt/$lte), $eq o igualdad escalar. null si no aplica.
function _sortedIndexLookup(index, cond) {
  if (cond === null || typeof cond !== 'object') return index.range(cond, cond);
  let min = -Infinity, max = Infinity, excludeMin = false, excludeMax = false;
...
class Cursor {
  ...
  toArray() {
    // OPT: escaneo dirigido por SortedIndex cuando se ordena por un único campo indexado.
    const indexed = this._indexedSortScan();
    if (indexed) return indexed;

    // Camino estándar: find + sort en memoria (refs crudas, sin clonar aún).
    let docs = this._col._findRaw(this._filter);
    if (this._sort) this._sortInPlace(docs);

    // OPT: skip + limit ANTES de clonar — solo clonamos lo que devolvemos.
    if (this._skip > 0) docs = docs.slice(this._skip);
    if (this._limit > 0) docs = docs.slice(0, this._limit);

    if (this._proj) return this._applyProjection(docs);
    return docs.map(_clone);
  }

  // Devuelve el slice clonado vía orden de índice, o null si el fast-path no aplica.
  _indexedSortScan() {
    if (!this._sort || this._proj) return null;
    const sortFields = Object.entries(this._sort);
    if (sortFields.length !== 1) return null;
    const [sortField, sortDir] = sortFields[0];
    const index = this._col._indexes.get(sortField);
    if (!(index instanceof SortedIndex)) return null;

    let docs = this._scanByIndex(index, sortDir > 0);
    if (this._skip > 0) docs = docs.slice(this._skip);
    if (this._limit > 0) docs = docs.slice(0, this._limit);
    return docs.map(_clone);
  }

  // Recorre el índice en orden, filtrando, con early-exit al alcanzar skip+limit.
  _scanByIndex(index, ascending) {
    const docs = [];
    const maxRequired = this._limit > 0 ? this._skip + this._limit : Infinity;
    for (const id of index.all(ascending)) {
      const doc = this._col._docs.get(id);
      if (doc && matchFilter(doc, this._filter)) {
        docs.push(doc);
        if (docs.length >= maxRequired) break;
      }
    }
    return docs;
  }
  ...
  _applyProjection(docs) {
    ...
    return docs.map(doc => includeMode ? this._projectInclude(doc) : this._projectExclude(doc));
  }

  _projectInclude(doc) {
    const result = {};
    if (this._proj._id !== 0) result._id = doc._id; // _id incluido salvo exclusión explícita
...
// Acumuladores de $group: cada fn recibe (groupDocs, fieldPath) y devuelve el valor agregado.
const GROUP_ACCUMULATORS = {
  $count: (docs)    => docs.length,
  $sum:   (docs, f) => docs.reduce((s, d) => s + (typeof f === 'string' ? (Number(_getNestedValue(d, f)) || 0) : (Number(f) || 0)), 0),
  $avg:   (docs, f) => {
    const vals = docs.map(d => typeof f === 'string' ? (Number(_getNestedValue(d, f)) || 0) : 0);
    return vals.length > 0 ? vals.reduce((a, b) => a + b, 0) / vals.length : 0;
  },
  $min:   (docs, f) => Math.min(...docs.map(d => typeof f === 'string' ? (Number(_getNestedValue(d, f)) || Infinity) : Infinity)),
  $max:   (docs, f) => Math.max(...docs.map(d => typeof f === 'string' ? (Number(_getNestedValue(d, f)) || -Infinity) : -Infinity)),
  $push:  (docs, f) => docs.map(d => typeof f === 'string' ? _getNestedValue(d, f) : undefined),
  $first: (docs, f) => (docs.length > 0 && typeof f === 'string') ? _getNestedValue(docs[0], f) : undefined,
  $last:  (docs, f) => (docs.length > 0 && typeof f === 'string') ? _getNestedValue(docs[docs.length - 1], f) : undefined,
};

// Resuelve los acumuladores de un grupo. Mismo orden de precedencia que la cadena
// if/else original: el primer operador presente gana.
function _resolveAccumulators(groupDocs, accumulators) {
  const result = {};
  for (const [accName, accDef] of Object.entries(accumulators)) {
    for (const op of Object.keys(GROUP_ACCUMULATORS)) {
      if (accDef[op]) { result[accName] = GROUP_ACCUMULATORS[op](groupDocs, accDef[op]); break; }
    }
  }
  return result;
}

function _aggGroup(docs, stage) {
  ...
  for (const [key, groupDocs] of groups) {
    out.push({ _id: key, ..._resolveAccumulators(groupDocs, stage.accumulators) });
  }
  return out;
}

function _aggSort(docs, stage) {
...
function _aggProject(docs, stage) {
  ...
  return docs.map(doc => {
    ...
    if (includeMode) {
      ...
      Object.assign(result, _clone(doc));
      for (const [k, v] of Object.entries(stage.spec)) {
...
function _aggUnwind(docs, stage) {
  const newDocs = [];
  for (const doc of docs) {
...
function _buildForeignIndex(foreignCol, foreignField) {
  const idx = new Map();
  for (const fDoc of foreignCol._docs.values()) {
    const fVal = _getNestedValue(fDoc, foreignField);
    if (fVal === undefined) continue;
    const key = String(fVal);
    if (!idx.has(key)) idx.set(key, []);
    idx.get(key).push(fDoc);
  }
  return idx;
}

function _aggLookup(docs, stage, col) {
  const store = col._store;
  ...
  const foreignIdx = _buildForeignIndex(foreignCol, stage.foreignField);
  ...
  for (const doc of docs) {
    ...
    const copied = matches.map(m => _clone(m));
    doc[stage.as] = stage.single ? (copied[0] || null) : copied;
  }
  return docs;
}

// Despacho de stages del pipeline: (docs, stage, col) -> docs transformados.
const STAGE_HANDLERS = {
  match:   (docs, stage) => docs.filter(d => matchFilter(d, stage.filter)),
  group:   (docs, stage) => _aggGroup(docs, stage),
  sort:    (docs, stage) => _aggSort(docs, stage),
  limit:   (docs, stage) => docs.slice(0, stage.n),
  skip:    (docs, stage) => docs.slice(stage.n),
  project: (docs, stage) => _aggProject(docs, stage),
  unwind:  (docs, stage) => _aggUnwind(docs, stage),
  lookup:  (docs, stage, col) => _aggLookup(docs, stage, col),
};
...
class AggregationPipeline {
  ...
  toArray() {
    ...
    for (const stage of this._stages) {
      const handler = STAGE_HANDLERS[stage.type];
      if (handler) docs = handler(docs, stage, this._col);
    }
    return docs;
  }
}
...
class Collection {
  ...
  _tryIndexLookup(filter) {
    ...
    for (const [field, cond] of Object.entries(filter)) {
      ...
      const ids = (index instanceof HashIndex) ? _hashIndexLookup(index, cond)
                : (index instanceof SortedIndex) ? _sortedIndexLookup(index, cond)
                : null;
      if (ids) return ids;
    }
...
const COLUMN_VALIDATORS = {
  ...
  datetime:    (v) => typeof v === 'string' || typeof v === 'number' || v instanceof Date,
...
// Tipos de columna auto-generados: se omiten en la validación de tipos.
const _AUTO_COLUMN_TYPES = new Set(['autonumber', 'formula']);
...
class Table {
  ...
  _runSyncHooks(event, ...args) {
    if (this._hooks && this._hooks[event]) {
      for (const cb of this._hooks[event]) {
        try {
          cb(...args);
        } catch (err) {
          console.error(`[Sync Hook Error on ${event}]:`, err);
          throw err;
        }
      }
    }
  ...
  _runAsyncHooks(event, ...args) {
    (async () => {
      try {
        const hookCol = this.db.collection('_hooks');
        const dbHooks = hookCol.find({ collection: { $in: [this.name, '*'] }, event, active: true }).toArray();
        for (const hook of dbHooks) queueMicrotask(() => this._dispatchHook(event, hook, args[0]));
      } catch (e) {
        // Silently catch if _hooks is not defined
      }
    })();
  }

  _dispatchHook(event, hook, data) {
    if (hook.action === 'webhook' && hook.url) {
      const payload = { event, collection: this.name, timestamp: new Date().toISOString(), data };
      fetch(hook.url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      })
        .then(res => { if (!res.ok) console.error(`[Async Webhook Hook Error]: status ${res.status}`); })
        .catch(err => console.error(`[Async Webhook Hook Error]:`, err));
    } else if (hook.action === 'log') {
      this._logHook(event, data);
    }
  }

  _logHook(event, data) {
    try {
      const auditCol = this.db.collection('_audit_logs');
      auditCol.insert({ collection: this.name, event, timestamp: new Date().toISOString(), details: data });
      auditCol.flush();
    } catch (err) {
      console.error(`[Async Log Hook Error]:`, err);
    }
  }
  ...
  _validate(doc, isUpdate = false) {
    const errors = [];
    for (const col of this.columns) {
      const err = this._validateColumn(col, doc[col.name], isUpdate);
  ...
  // Valida una columna; devuelve el mensaje de error o null.
  _validateColumn(col, val, isUpdate) {
    const absent = val === undefined || val === null;
    if (col.required && !isUpdate && (absent || val === '')) return `${col.name} is required`;
    if (absent || _AUTO_COLUMN_TYPES.has(col.type)) return null; // opcional ausente o auto-generado
    const validator = COLUMN_VALIDATORS[col.type];
    return (validator && !validator(val, col)) ? `${col.name}: invalid ${col.type} value` : null;
  }
  ...
  // ── CRUD (validated with hooks) ─────────────────────────

  insert(doc) {
    const withDefaults = this._applyDefaults(doc);
    this._runSyncHooks('beforeInsert', withDefaults);
    this._validate(withDefaults);
    const result = this._col.insert(withDefaults);
    this._runSyncHooks('afterInsert', result);
    this._runAsyncHooks('afterInsert', result);
    return result;
  }
  ...
  update(filter, update) {
    ...
    const payload = { filter, update };
    this._runSyncHooks('beforeUpdate', payload);
    const result = this._col.update(filter, update);
    this._runSyncHooks('afterUpdate', payload);
    this._runAsyncHooks('afterUpdate', payload);
    return result;
  }
  ...
  updateMany(filter, update) {
    ...
    const payload = { filter, update };
    this._runSyncHooks('beforeUpdate', payload);
    const result = this._col.updateMany(filter, update);
    this._runSyncHooks('afterUpdate', payload);
    this._runAsyncHooks('afterUpdate', payload);
    return result;
  }
  ...
  remove(filter)   {
    ...
    this._runSyncHooks('beforeDelete', payload);
    const result = this._col.remove(filter);
    this._runSyncHooks('afterDelete', payload);
    this._runAsyncHooks('afterDelete', payload);
    return result;
  }
  
  removeMany(f)    {
    const payload = { filter: f };
    this._runSyncHooks('beforeDelete', payload);
    const result = this._col.removeMany(f);
    this._runSyncHooks('afterDelete', payload);
    this._runAsyncHooks('afterDelete', payload);
    return result;
  }
...
// Reglas de política de contraseña: cada una devuelve el mensaje de error o null.
const PASSWORD_RULES = [
  (pw, p) => (p.minLength != null && pw.length < p.minLength) ? `Password must be at least ${p.minLength} characters` : null,
  (pw, p) => (p.maxLength != null && pw.length > p.maxLength) ? `Password must be at most ${p.maxLength} characters` : null,
  (pw, p) => (p.requireUppercase && !/[A-Z]/.test(pw)) ? 'Password must contain at least one uppercase letter' : null,
  (pw, p) => (p.requireLowercase && !/[a-z]/.test(pw)) ? 'Password must contain at least one lowercase letter' : null,
  (pw, p) => (p.requireDigit && !/\d/.test(pw)) ? 'Password must contain at least one digit' : null,
  (pw, p) => (p.requireSymbol && !/[^A-Za-z0-9]/.test(pw)) ? 'Password must contain at least one symbol' : null,
];
