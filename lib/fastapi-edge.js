import { validate, serialize } from './validation.js';
    ...
    async handle(request, env, ctx) {
        ...
        const executePipeline = async () => {
            ...
            if (pathname === '/openapi.json' && request.method === 'GET') {
                return buildResponse(await this._getOpenAPISchema());
            }
            ...
            if (matchedRoute.model) {
                const { valid, errors } = validate(rawBody, matchedRoute.model);
                if (!valid) {
                    return buildResponse({ detail: "Error de validación", errors }, 422);
                }
            }
