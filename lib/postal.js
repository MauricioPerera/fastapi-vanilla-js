...
async function verifyAuthorChain(from, list) {
    ...
    let expected = 0, prevHash = null, broken = false;
    const failures = [];
    for (const ev of sorted) {
        if (broken) { failures.push({ from, seq: ev.seq, reasons: ['chain-broken'] }); continue; }
        const reasons = [];
        ...
        if (reasons.length) { failures.push({ from, seq: ev.seq, reasons }); broken = true; continue; }
...
// Función auxiliar para ordenar eventos por created_at con desempate determinista por seq.
...
async function verifyTemporalProvenance(ev, keyLedger) {
    ...
    if (!keyState) {
        // Autor no registrado: anonimo-legacy si no trae firma (admitido, compat iter 3).
        // unknown-author solo si el evento SI trae firma de un autor desconocido.
        return hasSig(ev) ? ['unknown-author'] : [];
    }
