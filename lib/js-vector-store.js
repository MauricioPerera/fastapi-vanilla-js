...
const MATCH_OPERATORS = {
  ...
  $regex:  (val, target) => {
    const re = _safeUserRegex(target);
    if (re.global || re.sticky) re.lastIndex = 0; // evita el bug de estado al reutilizar regex global/sticky
...
const LOGICAL_OPERATORS = {
  $and: (metadata, sub) => Array.isArray(sub) && sub.every(s => matchFilter(metadata, s)),
  $or:  (metadata, sub) => Array.isArray(sub) && sub.some(s => matchFilter(metadata, s)),
  $not: (metadata, sub) => !matchFilter(metadata, sub),
};

/** Evalúa una condición de campo: igualdad simple u objeto de operadores. */
...
function matchFilter(metadata, filter) {
  ...
  for (const key of Object.keys(filter)) {
    if (key in LOGICAL_OPERATORS) {
      if (!LOGICAL_OPERATORS[key](metadata, filter[key])) return false;
      continue;
    }
    if (!_matchValue(metadata[key], filter[key])) return false;
  }
  return true;
}
...
class FileStorageAdapter {
  ...
  writeBin(filename, buffer) {
    const file = this.path.join(this.dir, filename);
    const tmp = file + '.tmp';
    this.fs.writeFileSync(tmp, Buffer.from(buffer));
    this.fs.renameSync(tmp, file); // rename atómico: nunca deja un .bin a medias ante un crash
  }
  ...
  writeJson(filename, data) {
    ...
    this.fs.renameSync(tmp, file); // rename atómico: el manifest nunca queda truncado
  }
...
class VectorStore {
  ...
  constructor(dirOrAdapter, dim = 768, maxCollections = 50, opts = {}) {
    ...
    this.hybrid = new HybridSearch(this, this.bm25, 'weighted');
  }
  ...
  _load(col) {
    ...
    // Auto-rellenar BM25
    if (!this.bm25._data.has(col)) {
      if (manifest && manifest.bm25) {
        this.bm25.deserialize(col, manifest.bm25, ids);
      } else {
        for (let i = 0; i < ids.length; i++) {
          const text = extractSearchableText(meta[i]);
          if (text) this.bm25.addDocument(col, ids[i], text);
        }
      }
    }
  ...
  set(col, id, vector, metadata = {}) {
    ...
    // Rellenar BM25
    const text = extractSearchableText(metadata);
    if (text) {
      this.bm25.addDocument(col, id, text);
    } else {
  ...
  _flushCol(col, entry) {
    ...
    const bm25State = this.bm25.serialize(col, entry.ids);
    if (bm25State) manifest.bm25 = bm25State;
...
class PolarQuantizedStore {
  ...
  _flushCol(col, entry) {
    if (entry.pending.length > 0) {
      ...
      for (let p = 0; p < entry.pending.length; p++) {
        dst.set(new Uint8Array(entry.pending[p].packed), (committed + p) * bpv);
      }
      entry.bin = newBuf;
      entry.pending = [];
    }
...
// ---------------------------------------------------------------------------
// ENCRYPTED STORAGE ADAPTER (AES-256-GCM, zero deps — usa Web Crypto API)
// ---------------------------------------------------------------------------
// Wrapper sobre cualquier adapter. Cifra JSON y binarios (.bin) antes de escribir,
// desencripta al leer. Compatible con Node, browser y Workers.
...
class EncryptedStorageAdapter {
  ...
  constructor(inner, key) {
    this.inner = inner;
    this._key  = key;
    this._cache = new Map();   // filename -> decrypted data (JSON or ArrayBuffer)
    this._pending = new Map(); // filename -> pending decrypted data to be encrypted on persist()
  }

  /**
   * Crea un EncryptedStorageAdapter derivando una key AES-256 del password.
   * @param {object} inner    Adapter interno
   * @param {string} password Password para derivar la key
   * @param {string} [salt]   Salt (default: 'js-vector-store-v1')
   * @returns {Promise<EncryptedStorageAdapter>}
   */
  static async create(inner, password, salt = 'js-vector-store-v1') {
    const crypto = EncryptedStorageAdapter._getCrypto();
    ...
    return new EncryptedStorageAdapter(inner, key);
  }
  ...
  static _getCrypto() {
    ...
    try {
      ...
      throw new Error('EncryptedStorageAdapter: Web Crypto API not available');
    }
  }
   ...
   */
  async preload(filenames) {
    if (this.inner && typeof this.inner.preload === 'function') {
      await this.inner.preload(filenames);
    }
      ...
      if (f.endsWith('.json')) {
        ...
        if (encrypted) {
          if (encrypted.__enc) {
            ...
            // Retrocompatibilidad
            this._cache.set(f, encrypted);
          }
        }
      } else if (f.endsWith('.bin')) {
        const encrypted = this.inner.readBin(f);
        if (encrypted) {
          try {
            const decrypted = await this._decryptBin(encrypted);
            this._cache.set(f, decrypted);
          } catch (err) {
            // Retrocompatibilidad: plano
            this._cache.set(f, encrypted);
          }
        }
      }
  ...
  readJson(filename) {
    ...
    if (encrypted.__enc) return null; // No precargado síncronamente
    return encrypted; // Retrocompatibilidad
  }

  writeJson(filename, data) {
    this._cache.set(filename, data);
    this._pending.set(filename, { type: 'json', data });
  }
  ...
  readBin(filename) {
    ...
    const encrypted = this.inner.readBin(filename);
    if (!encrypted) return null;
    return encrypted; // Retrocompatibilidad fallback
  }

  writeBin(filename, buffer) {
    this._cache.set(filename, buffer);
    this._pending.set(filename, { type: 'bin', data: buffer });
  }
  ...
  async listKeys() {
    if (this.inner && typeof this.inner.listKeys === 'function') {
      return await this.inner.listKeys();
    }
    return [];
  }

  async persist() {
    for (const [filename, entry] of this._pending) {
      if (entry.type === 'json') {
        const encrypted = await this._encrypt(entry.data);
        this.inner.writeJson(filename, { __enc: encrypted });
      } else if (entry.type === 'bin') {
        const encrypted = await this._encryptBin(entry.data);
        this.inner.writeBin(filename, encrypted);
      }
    }
    this._pending.clear();
    if (this.inner && typeof this.inner.persist === 'function') {
      await this.inner.persist();
    }
  }
}
...
function extractSearchableText(metadata) {
  if (!metadata) return '';
  if (typeof metadata === 'string') return metadata;
  const parts = [];
  if (metadata.text) parts.push(String(metadata.text));
  if (metadata.content) parts.push(String(metadata.content));
...
class BM25Index {
  ...
  /** Exporta el estado comprimido usando mapeo posicional de enteros. */
  serialize(col, ids) {
    const d = this._data.get(col);
    if (!d || d.docCount === 0) return null;
    ...
    for (const [term, postingsMap] of d.invertedIndex) {
      if (postingsMap.size === 0) continue;
      
      const termIdx = vocab.length;
      vocab.push(term);
      termToIndex.set(term, termIdx);

      const termPostings = [];
      for (const [docId, tf] of postingsMap) {
        const docIdx = idMap.get(docId);
        if (docIdx !== undefined) {
          termPostings.push([docIdx, tf]);
        }
      }
      postings.push(termPostings);
    }

    const lens = new Array(ids.length);
    for (let i = 0; i < ids.length; i++) {
      const docId = ids[i];
      if (d.docLengths.has(docId)) {
        lens[i] = d.docLengths.get(docId);
      } else {
        // lens[i] = null indica doc sin contenido textual indexable -> excluido del BM25 e ignorado al reconstruir avgdl
        lens[i] = null;
      }
    }

    return { vocab, lens, postings };
  }

  /** Importa el estado comprimido usando mapeo posicional de enteros. */
  deserialize(col, state, ids) {
    if (!state) return;
    const { vocab, lens, postings } = state;
    if (!vocab || !lens || !postings) return;
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      if (length !== null && length !== undefined) {
        d.docLengths.set(docId, length);
        d.totalTokens += length;
        d.docCount++;
      }
    }
    ...
    for (let t = 0; t < vocab.length; t++) {
      const term = vocab[t];
      const termPostings = postings[t] || [];
      if (termPostings.length === 0) continue;

      const postingsMap = new Map();
      for (const [docIdx, tf] of termPostings) {
        const docId = ids[docIdx];
        if (docId !== undefined) {
          postingsMap.set(docId, tf);
        }
      }
      d.invertedIndex.set(term, postingsMap);
    }
  }
