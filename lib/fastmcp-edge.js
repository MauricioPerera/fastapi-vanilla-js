...
export class FastMCPEdge {
    ...
    // Orden: kill switch estatico -> auth (sin I/O) -> [kill switch KV, rate-limit, tope].
    ...
    async _guard(request, env) {
        ...
        // 1) Kill switch estatico (env, sin I/O)
        if (env.MCP_DISABLED === '1' || env.MCP_DISABLED === true) {
            return this._deny(503, 'MCP deshabilitado');
        }

        // 2) Autenticacion Bearer (env, sin I/O). Se valida ANTES de tocar KV para que:
        //    (a) la auth no dependa de la disponibilidad de KV, y (b) el trafico anonimo no
        //    genere lecturas de KV (evita amplificacion de coste/DoS sobre tu namespace).
        ...
        // 3) Controles con I/O (kill switch en caliente, rate-limit, tope diario).
        //    Si el backend (KV / rate limiter) falla, se registra y se deja pasar (fail-open):
        //    un fallo transitorio de KV no debe tumbar el MCP ni convertir el guard en un 500.
        //    La auth ya se aplico arriba, asi que no se relaja la barrera de seguridad.
        try {
            // 3a) Kill switch instantaneo via KV (sin redeploy)
            ...
            // 3b) Rate limit (binding nativo de Workers Rate Limiting)
            ...
            // 3c) Tope diario sobre KV (autocap de coste). KV es eventual y el incremento no es
            ...
            console.error('[FastMCPEdge] guard backend error (fail-open):', e && e.message);
        }
