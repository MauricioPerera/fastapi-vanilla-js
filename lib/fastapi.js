...
// Límite default de body (bytes). Cubre bodies normales (todos los tests usan bodies
// chicos) y frena bodies intencionalmente enormes que agotarían memoria del proceso.
const DEFAULT_MAX_BODY_BYTES = 5 * 1024 * 1024; // 5 MB

// Error de body excedido: lleva status 413 y `expose` para que el exception handler
// default lo responda tal cual al cliente (no como un 500 genérico).
...
// Coerción plana por tipo para el validador de query/body simple (_validate).
// Cada fn devuelve { value } si coerciona, o { error } con el mensaje.
const FLAT_COERCERS = {
    number: (value, key) => {
        // Number() coerciona boolean/[]/strings vacías a 0/1; rechazarlos explícitamente.
        if (typeof value === 'boolean' || Array.isArray(value) || (typeof value === 'string' && value.trim() === '')) {
            return { error: `El campo '${key}' debe ser un número válido.` };
        }
        const num = Number(value);
        return isNaN(num) ? { error: `El campo '${key}' debe ser un número válido.` } : { value: num };
    },
    boolean: (value, key) => {
        if (value === 'true' || value === true || value === 1 || value === '1') return { value: true };
        if (value === 'false' || value === false || value === 0 || value === '0') return { value: false };
        return { error: `El campo '${key}' debe ser un booleano.` };
    },
    string: (value) => ({ value: String(value) }),
    array: coerceArray,
};

// Coercer plano para `type: 'array'` (validador de query/body simple).
// Valida que el valor sea array; con `rules.itemsType` coerciona cada elemento
// con el coercer plano del mismo nombre. No muta la entrada; no lanza.
// (Contrato CCDD: ccdd/flat-array-coercer/contract.md)
function coerceArray(value, key, rules) {
    if (!Array.isArray(value)) return { error: `El campo '${key}' debe ser un array.` };
    const itemsType = rules && rules.itemsType;
    if (!itemsType) return { value };
    const itemCoercer = FLAT_COERCERS[itemsType];
    if (!itemCoercer) return { value };
    const out = [];
    for (const item of value) {
        const r = itemCoercer(item, key);
        if (r.error) return { error: `El campo '${key}' contiene elementos inválidos (${itemsType}).` };
        out.push(r.value);
    }
    return { value: out };
}
...
class APIRouter {
    ...
    patch(pathStr, handler, options) { this._registerRoute('PATCH', pathStr, handler, options); }
...
class FastAPI {
    constructor(config = {}) {
        ...
        // Límite de body configurable; default 5 MB. 0/infinity desactiva el corte.
        this.maxBodyBytes = config.maxBodyBytes != null ? config.maxBodyBytes : DEFAULT_MAX_BODY_BYTES;

        // Manejador de excepciones por defecto
        this.defaultExceptionHandler = (req, res, err) => {
            if (!res.writableEnded && !res.headersSent) {
                // Errores con status explícito marcados como expuestos (p.ej. 413 de
                // body excedido) se responden tal cual al cliente. El resto: 500.
                if (err && err.expose && err.status) {
                    res.json({ detail: err.message }, err.status);
                } else {
    ...
    // Parseador asíncrono del cuerpo JSON con límite de bytes.
    // Acumula en buffers y corta APENAS el tamaño acumulado supera maxBodyBytes:
    // no espera a que termine de llegar el body ni a parsearlo. Al exceder, remueve
    // los listeners, pausa el stream (deja de consumir/processar memoria) y rechaza
    // con PayloadTooLargeError (413) — el handler de la ruta nunca llega a ejecutarse.
    _parseJsonBody(req) {
        return new Promise((resolve, reject) => {
            ...
            const onData = (chunk) => {
                if (aborted) return;
                size += chunk.length; // bytes del buffer (no depende de encoding)
                if (limit != null && limit > 0 && size > limit) {
                    aborted = true;
                    cleanup();
                    req.pause(); // detiene el consumo; no destruye el socket (res queda usable)
    ...
    _validate(data, schema) {
        ...
        for (const [key, rules] of Object.entries(schema)) {
            ...
            if (value === undefined || value === null) {
                ...
                else validated[key] = rules.default !== undefined ? rules.default : null;
                continue;
            }

            const coercer = FLAT_COERCERS[rules.type];
            if (!coercer) { validated[key] = value; continue; }

            const result = coercer(value, key, rules);
